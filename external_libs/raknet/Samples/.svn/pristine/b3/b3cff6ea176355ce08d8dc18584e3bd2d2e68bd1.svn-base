...
void DownloadTable()
{
	phpDirectoryServer2->DownloadTable("beammedown");
}
void UploadTable(RakNet::RakString gameName, unsigned short gamePort)
{
	phpDirectoryServer2->UploadTable("beammeup", gameName, gamePort, false);
}
void UploadAndDownloadTable(RakNet::RakString gameName, unsigned short gamePort)
{
	phpDirectoryServer2->UploadAndDownloadTable("beammeup", "beammedown", gameName, gamePort, false);
}
...
bool RunTest()
{
	...
	phpDirectoryServer2->UploadTable("beammeup", "FirstGameUpload", 80, false);
	if (HaltOnUnexpectedResult(rr=ReadResult(httpResult), RR_EMPTY_TABLE))
		{PrintHttpResult(httpResult); return false;}
	if (PassTestOnEmptyDownloadedTable()==false)
		{PrintHttpResult(httpResult); return false;}
	...
	phpDirectoryServer2->UploadTable("beammeup", "SecondGameUpload", 80, false);
	if (HaltOnUnexpectedResult(rr=ReadResult(httpResult), RR_EMPTY_TABLE))
	{PrintHttpResult(httpResult); return false;}
	if (PassTestOnEmptyDownloadedTable()==false)
		{PrintHttpResult(httpResult); return false;}
