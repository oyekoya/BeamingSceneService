...
void ReliabilityLayer::InitializeVariables( int MTUSize )
{
	...
	sendReliableOrReceiptMessageNumberIndex = 0;
...
bool ReliabilityLayer::Send( char *data, BitSize_t numberOfBitsToSend, PacketPriority priority, PacketReliability reliability, unsigned char orderingChannel, bool makeDataCopy, int MTUSize, CCTimeType currentTime, uint32_t receipt )
{
	...
	if ( reliability > RELIABLE_SEQUENCED_WITH_ACK_RECEIPT || reliability < 0 )
			...
			while ((int)BITS_TO_BYTES(allDatagramSizesSoFar)<retransmissionBandwidth)
			{
				...
				while ( IsResendQueueEmpty()==false )
				{
					...
					if ( internalPacket->nextActionTime < time )
					{
						...
						if (internalPacket->reliability==UNRELIABLE_WITH_ACK_RECEIPT || internalPacket->reliability==UNRELIABLE_SEQUENCED_WITH_ACK_RECEIPT)
						{
							PopListHead(false);
				...
				)
			{
					...
					//while ( sendPacketSet[ i ].Size() )
				{
						...
						)
					{
						...
						internalPacket->reliableMessageNumber=sendReliableOrReceiptMessageNumberIndex;
						internalPacket->nextActionTime = congestionManager.GetRTOForRetransmission()+time;
#if CC_TIME_TYPE_BYTES==4
						...
						sendReliableOrReceiptMessageNumberIndex++;
					}
...
{
	int index1 = sendReliableOrReceiptMessageNumberIndex & (uint32_t) RESEND_BUFFER_ARRAY_MASK;
	//	int index2 = (sendReliableOrReceiptMessageNumberIndex+(uint32_t)1) & (uint32_t) RESEND_BUFFER_ARRAY_MASK;
